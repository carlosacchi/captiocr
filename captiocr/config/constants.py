...
# Default install locations per platform; OCRProcessor searches further
# (bundled binary, PATH, registry/Homebrew locations) when these miss.
if sys.platform == 'win32':
    TESSERACT_CMD = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
    TESSDATA_PREFIX = r'C:\Program Files\Tesseract-OCR\tessdata'
elif sys.platform == 'darwin':
    # Homebrew (Apple Silicon / Intel) and MacPorts locations.
    ...
    TESSERACT_CMD = next(
        (p for p in TESSERACT_CANDIDATE_PATHS if os.path.exists(p)),
        TESSERACT_CANDIDATE_PATHS[0],
    )
    TESSDATA_PREFIX = str(Path(TESSERACT_CMD).parent.parent / 'share' / 'tessdata')
else:
...
MAIN_WINDOW_ALPHA = 1.0  # Fully opaque
...
# UI Configuration
CAPTURE_WINDOW_ALPHA = 0.3  # Semi-transparent - control frame visible, capture area distinguishable
CAPTURE_OVERLAY_ALPHA_MACOS = 0.3  # macOS: opacity of the control bar and border (capture area itself stays fully clear via per-pixel transparency)
CAPTURE_WINDOW_COLOR = 'yellow'  # Using white for transparent effect
CONTROL_FRAME_HEIGHT = 40  # pixels - increased to avoid overlap with capture area
SELECTION_WINDOW_ALPHA = 0.3
...
def get_user_data_path() -> Path:
    ...
    Resolution order:
      ...
      4. ``~/Library/Application Support/CaptiOCR`` on macOS.
      5. ``~/.captiocr`` on other platforms.
    ...
    if sys.platform == 'darwin':
        return Path.home() / "Library" / "Application Support" / "CaptiOCR"
...
# Read-only resources stay alongside the application binaries. In a frozen
# build (PyInstaller) the bundled copy inside the archive is preferred, which
# is the only location that exists inside a macOS .app bundle.
def _resolve_resources_dir() -> Path:
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        bundled = Path(sys._MEIPASS) / "resources"
        if bundled.exists():
            return bundled
