...
# Global hotkey support is optional: on macOS the 'keyboard' module requires
# root privileges, so it is skipped there and the app falls back to an
# in-app Tk binding for Ctrl+Q.
keyboard = None
...
class MainWindow:
    ...
    def __init__(self):
        ...
        if not hotkey_registered:
            # In-app fallback: Ctrl+Q works while any CaptiOCR window has focus
            self.root.bind_all('<Control-q>', lambda _e: self._on_ctrl_q_toggle())
            self.logger.info("Using in-app Ctrl+Q binding (global hotkey unavailable)")
    ...
    def _show_tesseract_options_dialog(self) -> None:
        ...
        if sys.platform == 'darwin':
            auto_install_label = "Install automatically (Homebrew)"
            browse_label = "Browse for tesseract..."
        else:
            auto_install_label = "Install automatically (winget)"
            browse_label = "Browse for tesseract.exe..."

        ttk.Button(
            btn_frame,
            text=auto_install_label,
    ...
    def _browse_for_tesseract(self) -> None:
        """Let the user manually locate the tesseract executable and initialize from it."""
        if sys.platform == 'win32':
            ...
            dialog_options = {
                'title': "Locate tesseract",
                'initialdir': '/opt/homebrew/bin' if sys.platform == 'darwin' else '/usr/bin',
            }
        ...
        if success:
            ...
            if sys.platform == 'darwin':
                hint = "Please try again or install Tesseract with: brew install tesseract"
            else:
                hint = "Please try again or install Tesseract from the UB Mannheim website."
            messagebox.showerror(
                "Invalid Tesseract",
                f"The selected file does not appear to be a working tesseract executable.\n\n"
                f"Path: {path}\n\n"
                f"{hint}"
            )
            self._show_tesseract_manual_url_dialog(
    ...
    def _create_widgets(self) -> None:
        ...
        # Start/Stop button (ColorButton: honors green/red bg on macOS too)
    ...
    def _configure_window(self) -> None:
        ...
        # Bring the app to the front and make it the active application on
        # startup (see _activate_macos for why this is needed on macOS)
        if sys.platform == 'darwin':
            self.root.after(100, self._activate_macos)

    def _activate_macos(self) -> None:
        """
        Make CaptiOCR the frontmost, active application on macOS.

        A Tk app that is not the active application swallows the first click
        on each of its windows just to activate itself, so buttons feel like
        they need a second click. Registering a normal activation policy via
        AppKit (available inside the bundled .app) fixes this properly; the
        lift/focus fallback at least raises the window when AppKit is absent
        (e.g. running from source without pyobjc).
        """
        try:
            ...
            if app is not None:
                ...
                self.logger.info("Activated app via AppKit")
        except Exception as e:
            self.logger.debug(f"AppKit activation unavailable: {e}")

        # Always raise/focus the window as a portable fallback
        try:
            self.root.lift()
            ...
            self.logger.debug(f"Window raise/focus failed: {e}")

    def _set_window_icon(self) -> None:
        """Set window icon (.ico on Windows, PNG via iconphoto elsewhere)."""
        try:
            if sys.platform == 'win32':
                ...
                for png_path in (
                    self.file_manager.get_resource_path("icon.png"),
                    APP_PATH / "captiocr_logo.png",
                ):
                    if png_path.exists():
...
© {datetime.now().year} - OCR caption capture solution"""
    ...
    def _show_privacy_security(self) -> None:
        ...
        disclosure = (
            ...
            "  (%LOCALAPPDATA%\\CaptiOCR on Windows, ~/Library/\n"
            "  Application Support/CaptiOCR on macOS). You can delete\n"
            "  them at any time.\n\n"
