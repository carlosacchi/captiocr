...
class SelectionWindow(BaseWindow):
    ...
    def show(self) -> None:
        ...
        # Right-click cancels: Button-3 on Windows/X11, Button-2 on macOS aqua
        self.window.bind('<Button-3>', self._on_cancel)
        self.window.bind('<Button-2>', self._on_cancel)

        # Double-click confirms — needed on macOS where borderless
        # (overrideredirect) windows may not receive keyboard events
...
Press Enter or double-click to confirm selection."""
        ...
        if self.monitor_manager:
            ...
            # Add cancel/confirm buttons on primary monitor
            ...
            if primary:
                ...
                confirm_button.place(x=btn_x - 130, y=btn_y)
        else:
            ...
            confirm_button.place(relx=0.85, rely=0.05, anchor=tk.NE)
    ...
    def _on_mouse_up(self, event) -> None:
        ...
        if self.rect_id and self.start_x is not None and self.start_y is not None:
            ...
            # Ignore plain clicks (no real drag) so a double-click used to
            # confirm does not overwrite a valid selection with a tiny rect
