...
class MonitorManager:
    ...
    def _detect_darwin_scale(self) -> None:
        """
        Detect the Retina backing scale factor on macOS.

        Tk reports coordinates in points while screenshots are taken in
        physical pixels, so the ratio between the two is needed to convert
        the selected area into screenshot coordinates. Taking a screenshot
        here also triggers the Screen Recording permission prompt early,
        at startup, instead of mid-capture.
        """
        ...
        try:
            ...
            if monitor.width > 0 and screenshot.width > 0:
                scale = screenshot.width / monitor.width
                if scale >= 1.0:
                    monitor.scale_factor = scale
                    monitor.dpi = int(round(self.DEFAULT_DPI * scale))
                    self.logger.info(
                        f"macOS backing scale detected: {scale:.2f} "
                        f"(screen {monitor.width}pt -> {screenshot.width}px)"
                    )
        except Exception as e:
            self.logger.warning(f"Could not detect macOS backing scale: {e}")
