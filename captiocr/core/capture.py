...
class ScreenCapture:
    ...
    def _grab_macos(self) -> Optional[Image.Image]:
        """
        Grab the capture area on macOS at full Retina resolution.

        The native `screencapture -R` tool takes global coordinates in
        points — the same units Tk uses — so the selection is passed
        unscaled. It is called directly (instead of via PIL.ImageGrab)
        because Pillow resizes the result back down to point size, which
        would halve the OCR resolution on Retina displays.

        The area is clamped to the visible screen so a window dragged
        partially off-screen does not make the grab fail.

        Returns:
            PIL Image of the region, or None if the grab failed/off-screen.
        """
        x1, y1, x2, y2 = self.capture_area
        if self.monitor_manager:
            vx, vy, vw, vh = self.monitor_manager.get_virtual_screen_bounds()
            x1, y1 = max(x1, vx), max(y1, vy)
            x2, y2 = min(x2, vx + vw), min(y2, vy + vh)

        if x2 - x1 < 10 or y2 - y1 < 10:
            self.logger.warning(
                f"Capture area {self.capture_area} is (almost) entirely off-screen, skipping frame"
            )
            return None
        ...
        try:
            subprocess.run(
                ['screencapture', '-x', '-R', f'{x1},{y1},{x2 - x1},{y2 - y1}', filepath],
                capture_output=True,
                timeout=15,
            )
            ...
            self.logger.error(f"screencapture grab failed: {e}")
            try:
                # Fallback: Pillow's grab (point-resolution, but works)
                return ImageGrab.grab(bbox=(x1, y1, x2, y2))
            except Exception as e2:
                self.logger.error(f"Fallback grab also failed: {e2}")
                return None
        finally:
            try:
                os.unlink(filepath)
        ...
        try:
            ...
            while not self.capture_stop_flag:
                try:
                    ...
                    # Check if we need multi-monitor capture (Windows only;
                    # macOS uses its own native grab path below)
                    needs_multi_monitor_capture = False

                    if sys.platform == 'darwin':
                        pass
                    elif self.monitor_manager and self.monitor_manager.has_multi_monitor():
                    ...
                    if sys.platform == 'darwin':
                        # Native grab: correct point coordinates, full Retina
                        # resolution, area clamped to the visible screen
                        screenshot = self._grab_macos()
                        if screenshot is None:
                            self.stop_event.wait(timeout=capture_interval)
                            continue
                    elif needs_multi_monitor_capture:
                        try:
