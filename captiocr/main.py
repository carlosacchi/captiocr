...
def set_dpi_awareness():
    ...
    import logging
    _log = logging.getLogger('CaptiOCR.DPI')

    try:
        DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2 = -4
        result = ctypes.windll.user32.SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2)
        if result:
            _log.info("DPI awareness set to PER_MONITOR_AWARE_V2")
            return
    except Exception as e:
        _log.debug(f"SetProcessDpiAwarenessContext failed: {e}")

    try:
        PROCESS_PER_MONITOR_DPI_AWARE = 2
        result = ctypes.windll.shcore.SetProcessDpiAwareness(PROCESS_PER_MONITOR_DPI_AWARE)
        if result == 0:  # S_OK
            _log.info("DPI awareness set to PROCESS_PER_MONITOR_DPI_AWARE")
            return
    except Exception as e:
        _log.debug(f"SetProcessDpiAwareness failed: {e}")

    try:
        result = ctypes.windll.user32.SetProcessDPIAware()
        if result:
            _log.info("DPI awareness set to basic DPI_AWARE")
            return
    except Exception as e:
        _log.debug(f"SetProcessDPIAware failed: {e}")

    _log.warning("Could not set DPI awareness - DPI detection may be inaccurate")
...
def main():
    ...
    try:
        ...
        try:
            ...
            if not error_shown:
                print("Error details written to captiocr_main_error.txt", file=sys.stderr)
